// src/components/KolShahr4_7kayatCarousel.jsx
...
export default function KolShahr4_7kayatCarousel({
  ...
  const items = useMemo(
    () =>
      raw
        .map((it) => {
          let id = "";
          if (it.youtube_id) id = it.youtube_id;
          else if (it.youtube_url) id = toYouTubeId(it.youtube_url);
          if (!id) return null;

          const t = it.title || title;
  ...
  const goToIndex = (nextIdx) => {
    ...
    setIndex(Math.max(0, Math.min(maxIndex, nextIdx)));
  };

  const onCardClick = (it) => setPlayer({ open: true, item: it });

  return (
    <section className="programs-section shorts-like-programs cami-prophecies-section" dir="rtl" aria-labelledby="kol-shahr-4-7kayat-title">
      <div className="programs-header">
        <h2 id="kol-shahr-4-7kayat-title" className="programs-title">{title}</h2>
      ...
      <div className={`carousel ${loading ? "is-loading" : ""}`}>
        ...
        {!loading && !err && items.length > 0 && (
          <div className="carousel-viewport">
            <div ref={trackRef} className="carousel-track" dir="rtl" style={{ "--perView": visible }}>
              {items.map((it) => (
                <article key={it._key} className="program-card shortseg-card">
                  <button type="button" className="card-link short-open-btn" onClick={() => onCardClick(it)} aria-label={it._title}>
            ...
            <button type="button" className="nav-btn nav-prev" aria-label="السابق" onClick={() => goToIndex(index - step)} disabled={!canPrev}>‹</button>
            <button type="button" className="nav-btn nav-next" aria-label="التالي" onClick={() => goToIndex(index + step)} disabled={!canNext}>›</button>
          </div>
        )}
