...
export default function MrayaAlrohPage() {
  ...
  useEffect(() => {
    ...
    const fetchFast = async () => {
      ...
      let merged = (fresh || [])
        .map((it) => {
          const id = it.youtube_id || toYouTubeId(it.youtube_url || it.youtubeId || "");
  ...
  const normalized = useMemo(() => {
    return (items || [])
      .map((it) => {
        const id = it._ytid || it.youtube_id || toYouTubeId(it.youtube_url || it.youtubeId || "");
  ...
  useEffect(() => {
    ...
    let target =
      ordered.find((it) => String(it.id) === String(focusId) || String(it._ytid) === String(focusId)) ||
  ...
  return (
    <div className="cami-page wrap">
      <h2 className="cami-heading">مرايا الروح</h2>
        ...
        <>
          <div className="cami-grid">
            {paginatedItems.map((it) => {
              const title = it.title || "مرايا الروح";
              return (
                <button
