...
const PAGE_SIZE = 15;
const MAX_PAGES = 2;
const LS_KEY = "sehaAfdal_cache_v1";
...
export default function SehaAfdalPage() {
  ...
  useEffect(() => {
    ...
    const fetchFast = async () => {
      const cb = Date.now();
      const url = `/api/content/seha-afdal?limit=${need}&_cb=${cb}`;
  ...
  return (
    <div className="cami-page wrap">
      <h2 className="cami-heading">صحة افضل لحياة افضل</h2>
        ...
        <>
          <div className="cami-grid">
            {paginatedItems.map((it) => {
              const title = it.title || "صحة افضل لحياة افضل ";
              return (
                <button
