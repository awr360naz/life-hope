...
const PAGE_SIZE = 15;
const MAX_PAGES = 2;
const LS_KEY = "sbahAlkher_cache_v1";
...
export default function SbahAlkherPage() {
  ...
  useEffect(() => {
    ...
    const fetchFast = async () => {
      const cb = Date.now();
      const url = `/api/content/sbah-alkher?limit=${need}&_cb=${cb}`;
        ...
        <>
          <div className="cami-grid">
            {paginatedItems.map((it) => {
              const title = it.title ||  " صباح الخير مع ايات ";
              return (
                <button
