// src/pages/WamdatRaw7yePage.jsx
import React, { useEffect, useMemo, useState } from "react";
import { useLocation } from "react-router-dom";
import "./CamiPropheciesPage.css"; // نفس الستايل (بدك نعمل CSS خاص بعدين؟)
...
const PAGE_SIZE = 15;
const MAX_PAGES = 2;
const LS_KEY = "WamdatRaw7ye_cache_v1";
...
export default function WamdatRaw7yePage() {
  ...
  useEffect(() => {
    ...
    const fetchFast = async () => {
      const cb = Date.now();
      const url = `/api/content/Wamdat_Raw7ye?limit=${need}&_cb=${cb}`;
  ...
  return (
    <div className="cami-page wrap">
      <h2 className="cami-heading">ومضات روحية</h2>
...
{paginatedItems.map((it) => {
  const title = it.title || "ومضات روحية";
