...
import { useSearchParams } from "react-router-dom"; // 👈 مهم
...
const PAGE_SIZE = 12;
const MAX_PAGES = 4;
const LS_KEY = "shortSegs_cache_v2";
...
export default function ShortSegmentsPage() {
  // 👈 هذه السطر هو تعريف searchParams
  const [searchParams] = useSearchParams();
  const focusId = searchParams.get("focus"); // /shorts?focus=ID
  ...
  // 👈 هنا السحر: لو إجينا من السيرش مع focus=ID → افتح هذا الشورت
  useEffect(() => {
    ...
    if (match) {
      setPlayer({ open: true, item: match });
    }
  }, [focusId, normalized]);
              ...
              return (
                ...
                >
                  ...
                  <div className="shortseg-body">
                    <h3 className="shortseg-title" title={title}>
                      {title}
                    </h3>
                  </div>
                </button>
