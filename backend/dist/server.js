...
app.get("/api/content/search", async (req, res) => {
    ...
    const dbg = { q: rawQ, pats, steps: [], errors: [] };
    if (!rawQ || !supabase) {
        if (debug)
            return res.json({ ...dbg, note: "no query or no supabase", results: [] });
        return res.json([]);
    }
    ...
    async function tryIlike(table, field, sel, type) {
        try {
            let acc = [];
            for (const p of pats) {
    ...
    if (merged.length === 0) {
        try {
            ...
            const pickText = (obj) => Object.values(obj || {})
                .filter((v) => typeof v === "string")
                .join(" ");
            const ok = (obj) => norm(pickText(obj)).includes(qNorm);
...
/* =========================
   ...
   ========================= */
async function handleGetProgramToday(req, res) {
...
/* ========= Handler ========= */
async function handleGetHomeThird(req, res) {
    const sb = getSupabase();
    if (!sb) {
        return res.status(500).json({ ok: false, error: "Supabase not configured (env missing)" });
    }
    const rawSort = String(req.params?.sort ?? req.query?.sort ?? "0").trim();
    ...
    try {
        ...
        if (!drafts) {
            rows = rows.filter((r) => {
                const pub = (typeof r.published === "boolean" ? r.published : null) ??
        ...
        const tsVal = (r) => (r?.updated_at || r?.created_at || "");
        rows.sort((a, b) => {
        ...
        const sortField = Number.isFinite(Number(r?.sort)) ? Number(r.sort)
...
async function fetchShortSegs(limit = 30) {
    const supabase = getSupabase();
...
const server = app
    ...
    console.log(`   PROGRAMS_TABLE=${PROGRAMS_TABLE} | THIRD_TABLE=${THIRD_TABLE} | PROGRAMS_CATALOG_TABLE=${PROGRAMS_CATALOG_TABLE}`);
})
    .on("error", (err) => {
    if (err?.code === "EADDRINUSE") {
        console.error(`❌ البورت ${APP_PORT} محجوز (EADDRINUSE).\n` +
