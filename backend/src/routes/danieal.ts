...
const CACHE_DIR =
  process.env.danieal_CACHE_DIR || path.join(process.cwd(), ".cache");
const CACHE_FILE = path.join(CACHE_DIR, "danieal_video.json");
...
function readDiskCache(): danieal_video[] {
  try {
...
function writeDiskCache(items: danieal_video[]) {
  try {
    ensureDir(CACHE_DIR);
...
async function fetchWithFallback(): Promise<danieal_video[]> {
