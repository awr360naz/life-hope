// backend/src/routes/kolShahr4_7kayat.ts
...
const CACHE_DIR =
  process.env.KOL_SHAHR_CACHE_DIR || path.join(process.cwd(), ".cache");
const CACHE_FILE = path.join(CACHE_DIR, "kol_shahr_4_7kayat.json");
...
function readDiskCache(): KolShahrVideo[] {
  try {
...
function writeDiskCache(items: KolShahrVideo[]) {
  try {
    ensureDir(CACHE_DIR);
    fs.writeFileSync(
      CACHE_FILE,
...
async function queryTry(opts: {
  ...
  let q = sb.from("kol_shahr_4_7kayat").select("*");
