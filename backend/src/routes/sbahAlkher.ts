// backend/src/routes/sbahAlkher.ts
...
const CACHE_DIR =
  process.env.SBAH_CACHE_DIR || path.join(process.cwd(), ".cache");
const CACHE_FILE = path.join(CACHE_DIR, "sbah_alkher.json");
...
function readDiskCache(): SbahVideo[] {
  try {
...
function writeDiskCache(items: SbahVideo[]) {
  try {
    ensureDir(CACHE_DIR);
    fs.writeFileSync(
      CACHE_FILE,
...
async function queryTry(opts: {
  ...
  let q = sb.from("sbah_alkher").select("*");
...
async function fetchWithFallback(limit: number): Promise<SbahVideo[]> {
...
router.get("/", async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.json({ ok: true, items: [] as SbahVideo[] });
  } catch (e: any) {
    if (memCache.items.length > 0) {
