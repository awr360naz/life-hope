// backend/src/routes/sehaAfdal.ts
...
const CACHE_DIR =
  process.env.SEHA_CACHE_DIR || path.join(process.cwd(), ".cache");
const CACHE_FILE = path.join(CACHE_DIR, "seha_afdal.json");
...
function readDiskCache(): SehaVideo[] {
  try {
...
function writeDiskCache(items: SehaVideo[]) {
  try {
    ensureDir(CACHE_DIR);
    fs.writeFileSync(
      CACHE_FILE,
...
async function queryTry(opts: {
  ...
  let q = sb.from("seha_afdal").select("*");
...
async function fetchWithFallback(limit: number): Promise<SehaVideo[]> {
...
router.get("/", async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.json({ ok: true, items: [] as SehaVideo[] });
  } catch (e: any) {
    if (memCache.items.length > 0) {
