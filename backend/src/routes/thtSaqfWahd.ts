// backend/src/routes/thtSaqfWahd.ts
...
const CACHE_DIR =
  process.env.THT_CACHE_DIR || path.join(process.cwd(), ".cache");
const CACHE_FILE = path.join(CACHE_DIR, "tht_saqf_wahd.json");
...
function readDiskCache(): ThtVideo[] {
  try {
...
function writeDiskCache(items: ThtVideo[]) {
  try {
    ensureDir(CACHE_DIR);
    fs.writeFileSync(
      CACHE_FILE,
...
async function queryTry(opts: {
  ...
  let q = sb.from("tht_saqf_wahd").select("*");
...
router.get("/", async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.json({ ok: true, items: [] as ThtVideo[] });
  } catch (e: any) {
    if (memCache.items.length > 0) {
