// backend/src/routes/al7ya_welamal.ts
...
const CACHE_DIR =
  process.env.al7ya_CACHE_DIR || path.join(process.cwd(), ".cache");
const CACHE_FILE = path.join(CACHE_DIR, "al7yaVideo.json");
...
function readDiskCache(): al7yaVideo[] {
  try {
...
function writeDiskCache(items: al7yaVideo[]) {
  try {
    ensureDir(CACHE_DIR);
    fs.writeFileSync(
      CACHE_FILE,
