...
/* ========== كاش ذاكرة + TTL صغير ========== */
const memCache = { items: [] as ShortSeg[], updatedAt: 0, ttlMs: 3 * 60 * 1000 };
const now = () => Date.now();

/* تطبيع خفيف لمفتاح الفيديو */
const normalize = (r: ShortSeg): ShortSeg => ({ ...r, video_url: r.video_url ?? r.url ?? null });

async function queryTry(
  opts: { withPublished?: boolean; orderBy?: "published_at" | "created_at" | "id"; limit: number }
): Promise<ShortSeg[]> {
  const sb = getSupabase();
  let q = sb.from("short_segments").select("*");
  if (opts.withPublished) q = q.eq("published", true);          // لو ما فيه published، Supabase بيرمي error بنمسكه فوق
  if (opts.orderBy) q = q.order(opts.orderBy as any, { ascending: false });
  const { data, error } = await q.limit(opts.limit);
  if (error) throw error;
  return (data ?? []).map(normalize);
}

async function fetchWithFallback(limit: number): Promise<ShortSeg[]> {
...
router.get("/api/content/short-segments", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const payload = { ok: true, items: [] as ShortSeg[] };
    // بشكل افتراضي: ما بنرجّع [] إلا إذا allowEmpty=1 (للإدارة/الاختبار)
    if (!allowEmpty) {
      return res.status(200).json({ ok: true, items: [] as ShortSeg[], note: "empty-but-allowed=false" });
    }
