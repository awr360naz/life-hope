// backend/src/routes/camiProphecies.ts
...
const CACHE_DIR = process.env.CAMI_CACHE_DIR || path.join(process.cwd(), ".cache");
...
function readDiskCache(): CamiVideo[] {
  try {
...
// عدّل اسم الجدول هنا حسب ما عندك: cami_videos / cami_prophesies ...
async function queryTry(opts: {
  ...
  let q = sb.from("cami_videos").select("*"); // ← غيّر الاسم لو مختلف
...
async function fetchWithFallback(limit: number): Promise<CamiVideo[]> {
  ...
  for (const a of attempts) {
    try {
      ...
      // نكمل عالمحاولة اللي بعدها
    }
  }
  return [];
}

router.get("/api/content/cami-prophecies", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!allowEmpty) {
      return res
        .status(200)
        .json({ ok: true, items: [] as CamiVideo[], note: "empty-but-allowed=false" });
    }
