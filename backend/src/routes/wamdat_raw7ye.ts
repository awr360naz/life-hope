// backend/src/routes/wamdat_raw7ye.ts
...
const CACHE_DIR =
  process.env.wamdat_CACHE_DIR || path.join(process.cwd(), ".cache");
const CACHE_FILE = path.join(CACHE_DIR, "wamdatVideo.json");
...
function readDiskCache(): wamdatVideo[] {
  try {
