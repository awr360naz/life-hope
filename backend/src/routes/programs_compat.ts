...
function normDay(input: string): Day | null {
  if (!input) return null;
  const d = (input[0].toUpperCase() + input.slice(1).toLowerCase()) as Day;
...
router.put("/week", async (req: Request, res: Response) => {
  const body = req.body ?? {};
  const programs: Partial<Program>[] = Array.isArray(body.programs) ? body.programs : [];
  ...
  const payload = programs.map((p) => {
    ...
    if (!day) throw new Error("Invalid/missing day in one of programs");
 ...
 */
router.patch("/day/:day", async (req: Request, res: Response) => {
  const day = normDay(String(req.params.day));
  if (!day) return res.status(400).json({ error: "Invalid day param" });
